...
func Trace(stacks [][]uintptr) []uintptr {
	markedStacksMap := make(map[uintptr]struct{})
	var (
		markedStacks []uintptr
		checkFn      func(pointer uintptr)
	)

	checkFn = func(pointer uintptr) {
		_, ok := markedStacksMap[pointer]
		if ok || pointer == 0 {
			return
		}
		markedStacksMap[pointer] = struct{}{}
		markedStacks = append(markedStacks, pointer)
		checkFn(*(*uintptr)(unsafe.Pointer(pointer)))
	}
